        ...
        <Table>
          ...
          <TableBody>
            {leaderboard.map((user) => (
              <TableRow 
                key={user.handle} 
                className={`hover:bg-neutral-50 transition-colors border-neutral-100 last:border-0 ${user.isCurrentUser ? "bg-neutral-100 hover:bg-neutral-100/80" : ""}`}
              >
                <TableCell className="text-center py-3">
                  {user.rank === 1 ? (
                    <Trophy className="size-5 text-amber-500 mx-auto" />
                  ) : user.rank === 2 ? (
                    <Medal className="size-5 text-neutral-400 mx-auto" />
                  ) : user.rank === 3 ? (
                    <Medal className="size-5 text-amber-700 mx-auto" />
                  ) : (
                ...
                <TableCell className="text-right pr-6 py-3">
                  <span className="inline-flex items-center gap-1 text-sm font-medium text-neutral-900 bg-white border border-neutral-200 px-1.5 py-0 rounded">
                    {user.streak} <span className="text-[10px]">🔥</span>
                  </span>
                </TableCell>
              </TableRow>
            ))}
